...
'''
I have edited the test_img.txt and the CULane dataset test.txt files to change the image path
also edited main.m script and removed the first ../
'''
import os
    ...
    with sess.as_default():
        ...
        for i in range(math.ceil(len(image_list) / batch_size)):
            ...
            for cnt, image_name in enumerate(paths):
                print(image_name)
                # TODO converted for windows
                parent_path = os.path.dirname(image_name)[15:]
